...
def get_resource_path(relative_path):
    """获取资源文件的正确路径，支持pyinstaller打包后的运行"""
    try:
        # PyInstaller打包后的临时目录
...
class DesktopPet(QWidget):
    ...
    def initPall(self):
        # 导入准备在托盘化显示上使用的图标
        icons = get_resource_path('tigerIcon.jpg')
        # 菜单项退出，点击后调用quit函数
        quit_action = QAction('退出', self, triggered=self.quit)
        quit_action.setIcon(QIcon(icons))
        # 菜单项显示，点击后调用showwin函数
        showing = QAction('显示宠物', self, triggered=self.showwin)
        # 新建一个菜单项控件
    ...
    def initPetImage(self):
        ...
        self.movie = QMovie(get_resource_path("normal/normal1.gif"))
        # 设置标签大小
        ...
        # 将宠物正常待机状态的动图放入pet1中
        self.pet1 = []
        normal_dir = get_resource_path("normal")
        for i in os.listdir(normal_dir):
            self.pet1.append(os.path.join(normal_dir, i))
        
        # 将宠物正常待机状态的对话放入dialog中
        self.dialog = []
        # 读取目录下dialog文件，指定UTF-8编码防止乱码
        with open(get_resource_path("dialog.txt"), "r", encoding="utf-8") as f:
            text = f.read()
            # 以\n 即换行符为分隔符，分割放进dialog中
            self.dialog = text.split("\n")
            # 过滤空行
            self.dialog = [line.strip() for line in self.dialog if line.strip()]

    # 获取对话框样式
    def getTalkStyle(self):
        return """
            font: bold 25pt '楷体';
            color: white;
            ...
            border-radius: 10px;
            padding: 5px 15px;
            border: 2px solid rgba(0, 100, 200, 1);
        """
    ...
    def randomAct(self):
        ...
        # condition不为0，转为切换特有的动作，实现宠物的点击反馈
            ...
            # 读取特殊状态图片路径
            self.movie = QMovie(get_resource_path("click/click.gif"))
            # 宠物大小
            self.movie.setScaledSize(QSize(200, 200))
            # 将动画添加到label中
            self.image.setMovie(self.movie)
    ...
    def randomPosition(self):
        # 使用QGuiApplication获取屏幕信息（QDesktopWidget已废弃）
